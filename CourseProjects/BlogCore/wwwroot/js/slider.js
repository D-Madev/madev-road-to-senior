...
$(document).ready(function () {
    try {
        const $tbl = $('#tblSliders');

        if ($tbl.length === 0) {
            console.warn('No se encontró la tabla #tblSliders en el DOM. DataTable no se inicializará.');
            return;
        }
        ...
        dataTable = $tbl.DataTable({
            ajax: {
                ...
                error: function (xhr, status, error) {
                    console.error('Error en GET /admin/sliders/GetAll:', status, error, xhr.responseText);
            ...
            columns: [
                ...
                { data: 'nombre', width: '5%' },
                {
                    data: 'estado',
                    render: (estadoActual) => {
                        estadoActual == true? "Activo" : "Inactivo";
                    },
                    width: '5%'
                },
                {
                    ...
                    render: function (data) {
                        ...
                        return `
                            <div class="text-center">
                                <a href="/Admin/Sliders/Edit/${data}" class="btn btn-sm btn-warning">
                                    <i class="far fa-edit"></i> Editar
                                </a>
                                &nbsp;
                                <button class="btn btn-sm btn-danger btn-delete" data-url="/Admin/Sliders/Delete/${data}" >
